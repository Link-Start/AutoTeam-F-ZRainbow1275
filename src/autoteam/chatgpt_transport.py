"""Optional lightweight HTTP transport for ChatGPT Team API calls."""
...
class CurlCffiChatGPTTransport:
    ...
    def __init__(
        ...
        *,
        ...
        for name, value in _build_session_cookie_pairs(session_token):
            self._session.cookies.set(name, value, domain="chatgpt.com", path="/", secure=True)
        if account_id:
    ...
    def request(self, method: str, path: str, *, headers: dict | None = None, body=None) -> dict[str, object]:
...
def build_chatgpt_transport(
    *,
    ...
    try:
        ...
        if mode == "curl_cffi":
            logger.warning("[ChatGPT] CHATGPT_API_TRANSPORT=curl_cffi 但 curl_cffi 未安装，回退 Playwright")
        else:

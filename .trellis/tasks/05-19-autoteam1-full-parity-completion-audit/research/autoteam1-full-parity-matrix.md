...
| --- | --- | --- | --- | --- |
| `2f8d52d fix:reduce-rotation-probe-interference` | reduce status polling and auto-check interference while rotation/fill tasks are running | `src/autoteam/api.py` supports `/api/status?fast=true`, skips live quota probes in fast mode, bounds CLIProxy health reads for fast polling, records task `progress_history`, and skips auto-check expensive probes when a task is active; `docs/api.md` and `tests/unit/test_api_status.py` cover the behavior | Absorbed by adapted design | Current repo preserved its cancel-signal ordering in `_run_task`, SSE/multi-master status fields, and safe status helpers instead of copying target `api.py` wholesale. |

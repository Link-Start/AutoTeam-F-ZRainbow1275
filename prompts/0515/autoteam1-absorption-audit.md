...
| `/api/status` 暴露资源快照 | 已吸收并加固 | `api.py:get_status()` 返回 `runtime_resources`；`_safe_runtime_resource_snapshot()` 保证采集异常不导致 status 500；`tests/unit/test_api_status.py` 通过 |
...
| `_launch_browser()` / browser fallback 半初始化失败清理 | 已吸收并加固 | `ChatGPTTeamAPI._launch_browser()` 失败时 `self.stop()`；`start_with_session()` 浏览器 fallback 导航/token/workspace 阶段失败也 `stop()`；`SessionCodexAuthFlow.start()` 启动失败也 stop；目标测试覆盖 |
| 注册/OAuth 裸 close 清理 | 已吸收并继续加固 | `manager._complete_registration()`、`manager._register_direct_once()`、`invite.run()`、`codex_auth.login_codex_via_browser()` 已去除裸 `browser.close()`；直接注册有 `try/finally`；OAuth 临时页用 `close_playwright_objects(page=...)` |
...
| transport 回退 Playwright | 已吸收并继续加固 | `_transport_response_requires_browser_fallback()`、`_direct_api_fetch()`；request 抛异常、HTML/challenge、auth fallback、401 refresh 后 retry 抛异常均关闭坏 transport 后回退浏览器；`tests/unit/test_chatgpt_transport.py` |
...
| signup profile / OAuth failure recovery | 已吸收并修正误判 | Round 3 深查发现此前“已贯穿”结论不准确：OAuth about-you 仍硬编码 `User / 1995-06-15 / 25`。Round 4 继续发现 `birthday` 嵌套 dict 可变且 OAuth about-you 伪成功。现已将 `SignupProfile` 从邀请注册/直接注册透传到 Team/Personal OAuth，冻结嵌套 birthday，并让 OAuth about-you 多顺序重试/失败返回 `None`，同时保留当前 Round 11/12 Personal OAuth retry、plan drift、workspace pool、register dual path |
...
- `python -m pytest tests/unit/test_free_registration_hardening.py tests/unit/test_round12_s3_cherry_pick.py tests/unit/test_round11_session_token_injection.py` -> `65 passed`
...
- 本轮复查时 `python -m ruff check src tests` 已全量通过；早前记录的 `tests/unit/test_round12_wireup.py` lint 失败不再是当前阻塞。
